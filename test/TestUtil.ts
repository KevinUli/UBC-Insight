...
/**
 * Removes all persisting dataset within the Validation
 */
export async function removeValidation(): Promise<void> {
	const facade = new InsightFacade();
	const datasetList = await facade.listDatasets();
	const promises: Promise<string>[] = [];
	datasetList.forEach((item: InsightDataset) => {
		promises.push(facade.removeDataset(item.id));
	});
